...
using static LanguageExtTestingConsoleApp.Program;
...
namespace LanguageExtTestingConsoleApp
{
    ...
    class Program
    {
        ...
        //    // user define transformation passed to bind(), this currently just ignores the extracted
        //    // contents in the Box and returns a new set of numbers as dictated by MyFunction()
        ...
        //    Box<string[]> transformedResult2x = numbers2.Map(contents => MyFunction3(contents));
        ...
        //    // The Box class is now considered a Monad, because it has these two additional function
        //    // s defined on it (map and bind).
        //    // Note in both cases of Bind() and Map() we did a transformation of the contents ie
        //    // we provided a function that would work on the item in the Box(or Monad)

        //    // In both cases we didn't doing anything with the contents of the box,
        //    // so we didn't really transform the contents(we just used what Map() and Bind()
        //    // extracted from the box)
        //    // We could have included the contents in our transformation and manipulated it... 

        //    // The take away is that Map() and Bind() do the same thing but Bind() requires you
        ...
        //}
        //private static string[] MyFunction3(int[] numbers)
        //{
        //    // Used as a map() transformation, so no need to lift into a Box or anything...
